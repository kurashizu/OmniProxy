...
fn route_table(ui: &mut egui::Ui, routes: &[RouteInfo]) {
    let def: Vec<&RouteInfo> = routes.iter()
    ...
    egui::Grid::new("route_grid").striped(true).min_col_width(w).show(ui, |ui| {
        ui.strong("Dest");
        ui.strong("Gateway");
        ui.strong("Iface");
        ui.end_row();
...
impl DashboardApp {
    pub(crate) fn overview_page(&mut self, ui: &mut egui::Ui) {
        ui.columns(2, |cols| {
            ...
            cols[0].add_space(4.0);
            kv_table(&mut cols[0], "traffic_grid", &[
